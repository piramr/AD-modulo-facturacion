...
export default function SeleccionCajaView({
  ...
  const cajasDisponibles = (cajas || []).filter((caja) => caja.estado === 'ACTIVO')
  const estaEnRevision = sesionActiva?.estado === 'EN_REVISION'
  const iniciales = user?.userName?.slice(0, 2)?.toUpperCase() || 'CJ'
  const cajaSeleccionada = cajasDisponibles.find((caja) => caja.id === sesionForm.cajaId)
              ...
              {cajasLoading ? (
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {cajasDisponibles.map((caja) => (
                    ...
                    >
                      <span className="text-sm font-bold text-slate-900 dark:text-slate-100">{caja.codigo}</span>
                      <span className="mt-1 block text-xs text-slate-500 dark:text-slate-400">{caja.descripcion}</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/55 p-4 backdrop-blur-sm">
          <div className="w-full max-w-sm overflow-hidden rounded-lg border border-slate-200 bg-white shadow-xl dark:border-slate-800 dark:bg-slate-950">
            <header className="flex items-start justify-between gap-4 border-b border-slate-200 px-5 py-4 dark:border-slate-800">
              <div>
                <h2 className="text-sm font-bold text-slate-900 dark:text-slate-100">Abrir caja</h2>
                <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
            ...
            <footer className="flex justify-end gap-2 border-t border-slate-200 bg-slate-50 px-5 py-3 dark:border-slate-800 dark:bg-slate-900">
              ...
              <button
                ...
                disabled={isSubmitting || !sesionForm.cajaId || sesionForm.montoApertura === ''}
