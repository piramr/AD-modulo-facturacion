          ...
          <nav className="flex-1 px-3 py-4">
            ...
            <div className="space-y-1">
              {navigation.map((item) => {
                ...
                return (
                  <NavLink key={item.to} to={item.to} end className={({ isActive }) => `flex items-center gap-3 rounded-2xl px-3 py-3 text-sm font-semibold transition-colors ${sidebarOpen ? '' : 'justify-center'} ${isActive ? 'bg-red-50 text-red-800 dark:bg-red-950/40 dark:text-red-300' : 'text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800/70'}`} title={!sidebarOpen ? item.label : undefined}>
          ...
          <header className="border-b border-slate-200 bg-white px-6 py-4 dark:border-slate-800 dark:bg-slate-900">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex items-center gap-4">
                ...
                <div><h1 className="flex items-center gap-2 text-lg font-black tracking-tight sm:text-xl"><span>Facturación</span><ChevronRight className="h-4 w-4 text-slate-400" /><span className="text-red-700 dark:text-red-500">{currentSection}</span></h1></div>
              </div>
